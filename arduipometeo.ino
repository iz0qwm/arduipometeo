...
// Tempo di aggiornamento della velocit� del vento
...
RTC_DS3231 rtc; //Make a RTC DS3231 object per settare l'orario
...
void setup()
{
  ...
  if (!SD.begin(4)) {// Usiamo il pin 4 dove c'� il CS per capire se la scheda � inserita
...
String dataora() {
    //Leggi ora e data
    String data = rtca.getDateStr();
    String separatore = ",";
    String ora = rtca.getTimeStr();
    String data_ora = data + separatore + ora;
    //Serial.println(data_ora);
    return data_ora;
}
String calcTempHumidity() {
    //Variabili per termometro
    float hum;  //Valore umidit� DHT
    float tempOut; //Valore temperatura DHT
    float tempIn;//Valore temperatura DS3231
    dht.begin();
    tempIn = rtca.getTemp();
    //Leggi i dati di temperatura e umidit� e metti nelle variabili
